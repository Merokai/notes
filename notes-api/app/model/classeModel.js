...
classe.create = function (classe, result) {
        sql.query("INSERT INTO classe(intituleClasse) VALUES(?)", classe.intituleClasse, function (err, res) {
...
classe.update = function(classe, result){
  sql.query("UPDATE classe SET intituleClasse = ? WHERE idClasse = ?", [classe.intituleClasse, classe.idClasse], function (err, res) {
          if(err) {
              console.log("error: ", err);
                result(null, err);
             }
...
classe.remove = function(classe, result){
     sql.query("DELETE FROM classe WHERE idClasse = ?", classe.idClasse, function (err, res) {
